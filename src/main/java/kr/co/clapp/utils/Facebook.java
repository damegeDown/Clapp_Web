...
public class Facebook extends DefaultFacebook {
	 ...
	 * 
	 * @param APIKey
	 *            Facebook apllcation API key, User access_token
	 * @param request
	 * @return
	 */
	...
	public JsonObject getUserInfo(String user_id) throws Exception {
		URL url = new URL(GRAPH_URL + user_id + "?access_token="
				+ fb_info.get("access_token") + "&callback=?");
		...
		String meString = me.substring(me.indexOf("?(") + 2, me.indexOf(");"));
		//JsonObject my_info = (JsonObject) JSONSerializer.toJSON(meString);
		JsonParser parser = new JsonParser();
		JsonObject my_info = (JsonObject)parser.parse(meString);
		return my_info;
	}
	 ...
	 * @throws Exception
	 */
	public JsonObject getUserSpecificInfo(String user_id, String content_name)
			throws Exception {
		return defaultReceive(GRAPH_URL + user_id + "/" + content_name
	...
	public JsonObject getNewsFeed() throws Exception {
		return defaultReceive(GRAPH_URL + "me/home?access_token="
	...
	public JsonObject getProfileFeed(String user_id) throws Exception {
		return defaultReceive(GRAPH_URL + user_id + "/feed?access_token="
				+ fb_info.get("access_token"));
	}
	 ...
	 * 
	 * @param data
	 *            keys : Facebook Feed API 에서 제공하는 파라미터 값들
	 * @return 페이스북에 포스팅 된 message_id
	 * @throws Exception
	 */
	public JsonObject sendPostToMyWall(Map<String, String> data)
			throws Exception {
	...
	public JsonObject getLikes(String content_id) throws Exception {
		return defaultReceive(GRAPH_URL + content_id + "/likes?access_token="
				+ fb_info.get("access_token"));
	}
	 ...
	 * 
	 * @param data
	 *            keys : Facebook API에서 제공되는 파라미터 값들
	 * @return
	 * @throws Exception
	 */
	public JsonObject sendComment(String post_id, Map<String, String> data)
			throws Exception {
	 ...
	 * 
	 * @param data
	 *            keys : Facebook event API 에서 제공하는 파라미터 값들
	...
	public JsonObject getAlbum(String album_id) throws Exception {
		return defaultReceive(GRAPH_URL + album_id + "?access_token="
				+ fb_info.get("access_token"));
	}

	/**
	 * @param data
	 *            Facebook에서 제공되는 파라미터 값들, imagePath, message 필수
	 * @return photo id
	 * @throws Exception
	 */
	public JsonObject sendPhoto(Map<String, String> data) throws Exception {
		...
		file_post.getParams().setBooleanParameter(
		...
		try {
			...
			file_post.setRequestEntity(new MultipartRequestEntity(parts,
					file_post.getParams()));
	...
	public JsonObject getPhoto(String photo_id) throws Exception {
		return defaultReceive(GRAPH_URL + photo_id + "?access_token="
				+ fb_info.get("access_token"));
	}
	...
	public JsonObject getNote(String note_id) throws Exception {
		return defaultReceive(GRAPH_URL + note_id + "?access_token="
				+ fb_info.get("access_token"));
	}
	...
	public JsonObject getMyGroupId() throws Exception {
		return defaultReceive(GRAPH_URL + "me/groups?access_token="
				+ fb_info.get("access_token"));
	}
	 ...
	 *             add YBS
	 */
	public JsonObject sendPostToMyGroup(Map<String, String> data)
			throws Exception {
	 ...
	 *             add YBS
	 */
	public JsonObject sendPostToMyFriend(Map<String, String> data)
			throws Exception {
		return defaultSend(GRAPH_URL + fb_info.get("appkey") + "/feed", data);
	}
	...
	public JsonObject getFriendLists() throws Exception {
		return defaultReceive(GRAPH_URL + "me/friends?access_token="
				+ fb_info.get("access_token"));
	}
