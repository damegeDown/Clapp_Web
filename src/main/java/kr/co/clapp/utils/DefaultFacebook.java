...
public class DefaultFacebook {
	...
	protected DefaultFacebook(String APIKey, String access_token)
			throws Exception {
		fb_info = new HashMap<String, String>();
		fb_info.put("access_token", access_token);
		fb_info.put("appkey", APIKey); // 추가된 부분
	}
	 ...
	 * @throws Exception
	 */
	protected JsonObject defaultSend(String url, Map<String, String> data)
			throws Exception {
		...
		params[0] = new NameValuePair("access_token",
		...
		JsonObject result_json = readReturnMessage(post
	 ...
	 * 
	 * @param data
	 *            페이스북 API에서 지원하는 데이터들
	 * @return URLConnection으로 보낼 때 파라이터 형식
	 * @throws Exception
	 */
	protected StringBuffer setParameters(Map<String, String> data)
			throws Exception {
		Iterator<String> data_key = data.keySet().iterator();
		StringBuffer parameters = new StringBuffer();
		parameters.append(URLEncoder.encode("access_token", charset));
		parameters.append("=");
		parameters.append(URLEncoder.encode(fb_info.get("access_token"),
				charset));
		parameters.append("&");
		while (data_key.hasNext()) {
			String key = data_key.next();
	...
	public String replaceFacebookTimeForm(String date, String hour) {
		...
		if (start_hour.substring(start_hour.length() - 2).equals("pm")) {
			int tmp = Integer.parseInt(start_hour.substring(0,
					start_hour.indexOf(":"))) + 12;
			start_hour.replace(0, start_hour.indexOf(":"), String.valueOf(tmp));
		}
		start_hour.delete(start_hour.length() - 2, start_hour.length());
		fb_date_form.append(start_date[2]).append("-").append(start_date[1])
				.append("-").append(start_date[0]).append("T")
				.append(start_hour).append(":00");
	...
	protected JsonObject readReturnMessage(InputStream in) throws Exception {
		...
		//JSONObject result_json = (JSONObject) JSONSerializer.toJSON(resultString);
		JsonParser parser = new JsonParser();
		JsonObject result_json = (JsonObject)parser.parse(resultString);
		return result_json;
	}
}