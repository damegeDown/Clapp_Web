...
<div class="sub-content">
  <div>
	...
	<div class="search-box">
		<form id="searchForm" action="">
		  <input type="hidden" name="actionUrl" value="${contextPath }/excel/mailReport"/>
  ...
  <div style="margin:5px; color:red">* 메일발송 통계는 월간으로만 검색만 가능합니다.<br/>* 발송 수 아래(숫자)는 오픈 수 입니다.</div>
  ...
  <c:set var="statisticsLength" value="${statisticsEntity.statisticsList.size() }"/>
	<table class="board-list-r">
		...
		<c:if test="${statisticsLength == 0 }">
			<tbody>
				<td colspan="16" class="al-center">조회한 내용이 없습니다.</td>
			</tbody>
		</c:if>
		<c:if test="${statisticsLength > 0}">
		<tbody>
