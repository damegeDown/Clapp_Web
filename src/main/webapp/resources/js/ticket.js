                ...
                success : function(data) {
                    ...
                    if(dataLen > 0) {
                        ...
                        $.ajax({
                            ...
                            type: "post",
                            dataType: "json",
                            data: {userMasterKey: uMasterKey},
                            success: function (data) {
                                $(".serviceProductName").html("");
                                var dataLen = data.dataList.length;
                                if(dataLen > 0) {
                                    var selectList ="<option value=''>선택</option>";
                                    for (var i = 0; i < dataLen; i++) {
                                        selectList +="<option value='"+data.dataList[i].userTicketMasterKey+"'>"+data.dataList[i].productName+"</option>";
                                    }
                                    $(".serviceProductName").html(selectList);
                                }
                            }
                        });
                    } else {
